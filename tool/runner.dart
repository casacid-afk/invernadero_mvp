...
void _validarSiembraSmoke(MotorInvernadero motor) {
  ...
  final stockInicialSemillero = motor.calcularStockPorEtapa(
    Etapa.semillero_calefaccionado,
  );
  final stockInicialLechuga = motor.calcularStockPorCultivo(
    CultivoKeys.lechuga,
  );
  ...
  final stockFinalSemillero = motor.calcularStockPorEtapa(
    Etapa.semillero_calefaccionado,
  );
  ...
  if (stockFinalSemillero != stockInicialSemillero + 50) {
    ...
    print(
      '  Esperado: ${stockInicialSemillero + 50}, Obtenido: $stockFinalSemillero',
    );
    exit(1);
  }
  ...
  if (stockFinalLechuga != stockInicialLechuga + 50) {
    ...
    print(
      '  Esperado: ${stockInicialLechuga + 50}, Obtenido: $stockFinalLechuga',
    );
    exit(1);
  }
  ...
  try {
    ...
    if (e.toString().contains('cultivoKey inválida')) {
      ...
      print(
        'ERROR - Se esperaba ArgumentError por cultivoKey inválida, pero se obtuvo: $e',
      );
      exit(1);
    }
  }
  ...
  try {
    ...
    if (e.toString().contains('cantidad debe ser mayor a 0')) {
      ...
      print(
        'ERROR - Se esperaba ArgumentError por cantidad inválida, pero se obtuvo: $e',
      );
      exit(1);
    }
  }
...
void _validarTraspasoSmoke(MotorInvernadero motor) {
  ...
  final stockInicialSemilleroAntes = motor.calcularStockPorEtapa(
    Etapa.semillero_calefaccionado,
  );
  final stockInicialCrianzaAntes = motor.calcularStockPorEtapa(
    Etapa.bandeja_crianza,
  );
  final stockInicialLechugaAntes = motor.calcularStockPorCultivo(
    CultivoKeys.lechuga,
  );
  ...
  if (lotePrueba.cantidadActual != 100 ||
    ...
    print(
      '  Obtenido: ${lotePrueba.cantidadActual} unidades en ${lotePrueba.etapaActual.name}, activo: ${lotePrueba.activo}',
    );
    exit(1);
  }
  ...
  final stockInicialCrianza = motor.calcularStockPorEtapa(
    Etapa.bandeja_crianza,
  );
  final stockInicialLechuga = motor.calcularStockPorCultivo(
    CultivoKeys.lechuga,
  ...
  if (stockInicialSemillero != stockInicialSemilleroAntes + 100 ||
    ...
    print(
      '  Esperado: semillero=${stockInicialSemilleroAntes + 100}, lechuga=${stockInicialLechugaAntes + 100}',
    );
    print(
      '  Obtenido: semillero=$stockInicialSemillero, lechuga=$stockInicialLechuga',
    );
    exit(1);
  }
  ...
  if (stockFinalSemillero != stockInicialSemillero - 60) {
    ...
    print(
      '  Esperado: ${stockInicialSemillero - 60}, Obtenido: $stockFinalSemillero',
    );
    exit(1);
  }
  ...
  if (stockFinalCrianza != stockInicialCrianza + 60) {
    ...
    print(
      '  Esperado: ${stockInicialCrianza + 60}, Obtenido: $stockFinalCrianza',
    );
    exit(1);
  }
  ...
  final sublotesEnCrianza = motor.lotes
      .where(
        (lote) =>
  ...
  if (sublotesEnCrianza.isEmpty) {
    print(
      'ERROR - No se encontró sublote creado en crianza después de traspaso parcial',
    );
    exit(1);
  }
  ...
  if (loteOrigen.cantidadActual != 40 ||
      ...
      !loteOrigen.activo) {
    print(
      'ERROR - Lote origen no quedó con cantidad/etapa correcta después de traspaso parcial',
    );
    print('  Esperado: 40 unidades en semillero, activo');
    print(
      '  Obtenido: ${loteOrigen.cantidadActual} unidades en ${loteOrigen.etapaActual.name}, activo: ${loteOrigen.activo}',
    );
    exit(1);
  }
  ...
  try {
    ...
    print(
      'ERROR - El traspaso con cantidad mayor a disponible debería haber fallado',
    );
    exit(1);
  } catch (e) {
    // Esperado: debe lanzar ArgumentError
    if (e.toString().contains('No se puede traspasar') ||
      ...
      print(
        'ERROR - Se esperaba ArgumentError por cantidad excedida, pero se obtuvo: $e',
      );
      exit(1);
    }
  }
  ...
  try {
    ...
    if (e.toString().contains('No se puede traspasar') ||
      ...
      print(
        'ERROR - Se esperaba ArgumentError por orden de etapas inválido, pero se obtuvo: $e',
      );
      exit(1);
    }
  }
...
void _validarCosechaSmoke(MotorInvernadero motor) {
  ...
  if (loteEnFinal.etapaActual != Etapa.bancada_final ||
      ...
      !loteEnFinal.activo) {
    print(
      'ERROR - El lote no está en bancada_final con las características esperadas',
    );
    print('  Esperado: bancada_final, 100 unidades, activo');
    print(
      '  Obtenido: ${loteEnFinal.etapaActual.name}, ${loteEnFinal.cantidadActual} unidades, activo: ${loteEnFinal.activo}',
    );
    exit(1);
  }
  ...
  motor.crearCosecha(loteId: loteEnFinal.id, cantidad: 30, fecha: fechaCosecha);
  ...
  if (stockFinalLechuga != stockInicialLechuga - 30) {
    ...
    print(
      '  Esperado: ${stockInicialLechuga - 30}, Obtenido: $stockFinalLechuga',
    );
    exit(1);
  }
  ...
  final lotesEnCrianza = motor.lotes.where(
    (lote) =>
        lote.cultivoKey == CultivoKeys.rucula &&
        lote.etapaActual == Etapa.bandeja_crianza &&
        lote.activo == true &&
        lote.cantidadActual == 50,
  );
  ...
  if (lotesEnCrianza.isEmpty) {
    print(
      'ERROR - No se encontró lote de rucula en crianza después del traspaso',
    );
    exit(1);
  }
  ...
  if (lotesEnInicialRucula.isEmpty) {
    print(
      'ERROR - No se encontró lote de rucula en bancada_inicial después del traspaso',
    );
    exit(1);
  }
  ...
  if (lotesEnFinalRucula.isEmpty) {
    print(
      'ERROR - No se encontró lote de rucula en bancada_final después del traspaso',
    );
    exit(1);
  }
  ...
  for (int i = 1; i <= 4; i++) {
    ...
    if (i < 4) {
      ...
      if (stockRucula != stockInicialRucula - 50) {
        print(
          'ERROR - Stock de rucula incorrecto después del 4to corte (lote cerrado)',
        );
  ...
  try {
    ...
    print(
      'ERROR - El 5to corte debería haber fallado (lote inactivo o máximo de cortes alcanzado)',
    );
    exit(1);
  } catch (e) {
    ...
    if (e.toString().contains('inactivo') ||
      ...
      print(
        'ERROR - Se esperaba error por lote inactivo o máximo de cortes, pero se obtuvo: $e',
      );
      exit(1);
    }
  }
